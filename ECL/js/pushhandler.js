...
app.pushHandler = function() {
    ...
    pub.createChannelCallback = function(result, token) {
        if (result == blackberry.push.PushService.SUCCESS) {
            ...
            if (result == blackberry.push.PushService.INTERNAL_ERROR) {
                alert("Error: An internal error occurred during "
                      + "the create channel. Try registering again.");
            } else if (result == blackberry.push.PushService.CREATE_SESSION_NOT_DONE) {
                alert("Error: No call to blackberry.push.PushService.create "
                ...
                alert("Error: A port could not be obtained from the "
