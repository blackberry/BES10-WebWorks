 ...
 * PostHttp::post
 *
 * Make a network request to http://shikhir.com/LocationSave.php with POST data and get
 * the response
 */
//! [1]
void PostHttp::post(const QString &body)
{
    const QUrl url("http://shikhir.com/LocationSave.php");

    QNetworkRequest request(url);
    request.setHeader(QNetworkRequest::ContentTypeHeader, "text/plain");

    if (AppSettings::isUsingHttps()) {
        request.setUrl(QUrl("http://shikhir.com/LocationSave.php"));
